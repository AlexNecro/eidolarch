...
async function loadPhoto(id,which=tab){
  ...
  const originalUrl=`/api/photos/${photoId}/original?v=2.3.0&rid=${token}`;
...
async function setTab(which){
  ...
  try{
    ...
    let header='';
    if(which==='similar')header=`<div class="side-section-title">${esc(t('viewer.referencePhoto'))}</div><div class="viewer-reference"><img src="/api/photos/${photoId}/thumbnail?v=2.3.0"><span>${esc($('#caption').textContent||'#'+photoId)}</span></div><div class="side-section-title">${esc(t('viewer.similarResults'))}</div>`;
...
window.openChildViewer=async(id,which,ids)=>{const q=`tab=${encodeURIComponent(which)}&ctx=${encodeURIComponent(which)}&ids=${encodeURIComponent((ids||[]).join(','))}`;try{await api(`/api/window/viewer/${id}?${q}`,{method:'POST'})}catch{open(`/viewer?photo=${id}&${q}&v=2.3.0`,'_blank','popup,width=1500,height=950')}};

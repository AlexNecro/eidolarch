...
async function loadLocale(){
  ...
  setTooltip($('#zoomOut'),t('viewer.zoomOut'));setTooltip($('#zoomIn'),t('viewer.zoomIn'));$('#actualSize').textContent=t('viewer.actual');$('#fitImage').textContent=t('viewer.fit');setTooltip($('#actualSize'),t('viewer.actualHint'));setTooltip($('#fitImage'),t('viewer.fitHint'));setTooltip($('#fullscreenBtn'),t('viewer.fullscreen'));setTooltip($('#toggleSide'),t(sideHidden?'viewer.showPanel':'viewer.hidePanel'));
...
async function loadPhoto(id,which=tab){
  ...
  const originalUrl=`/api/photos/${photoId}/original?v=2.2.15&rid=${token}`;
...
async function setTab(which){
  ...
  try{
    ...
    let header='';
    if(which==='similar')header=`<div class="side-section-title">${esc(t('viewer.referencePhoto'))}</div><div class="viewer-reference"><img src="/api/photos/${photoId}/thumbnail?v=2.2.15"><span>${esc($('#caption').textContent||'#'+photoId)}</span></div><div class="side-section-title">${esc(t('viewer.similarResults'))}</div>`;
...
window.openChildViewer=async(id,which,ids)=>{const q=`tab=${encodeURIComponent(which)}&ctx=${encodeURIComponent(which)}&ids=${encodeURIComponent((ids||[]).join(','))}`;try{await api(`/api/window/viewer/${id}?${q}`,{method:'POST'})}catch{open(`/viewer?photo=${id}&${q}&v=2.2.15`,'_blank','popup,width=1500,height=950')}};
...
function applyTransform(){const img=$('#photo');if(!img.naturalWidth||!img.naturalHeight)return;const z=currentScale();img.style.maxWidth='none';img.style.maxHeight='none';img.style.width=img.naturalWidth+'px';img.style.height=img.naturalHeight+'px';img.style.transform=`translate(${panX}px,${panY}px) scale(${z})`;$('#fitImage').classList.toggle('active',zoom===0);$('#actualSize').classList.toggle('active',zoom!==0&&Math.abs(z-1)<.001);$('#stage').classList.toggle('pannable',z>naturalFitScale()+.001);if(pinnedDetection)requestAnimationFrame(()=>highlightDetection(pinnedDetection,false))}

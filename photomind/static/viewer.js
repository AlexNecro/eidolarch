...
async function loadLocale(){
  ...
  $('#zoomOut').title=t('viewer.zoomOut');$('#zoomIn').title=t('viewer.zoomIn');$('#actualSize').textContent=t('viewer.actual');$('#fitImage').textContent=t('viewer.fit');$('#fullscreenBtn').title=t('viewer.fullscreen');$('#toggleSide').title=t('viewer.hidePanel');$('#stage').title=t('viewer.zoomHint');
...
async function loadPhoto(id,which=tab){
  ...
  const originalUrl=`/api/photos/${photoId}/original?v=2.2.10&rid=${token}`;
...
async function setTab(which){
  ...
  try{
    ...
    let header='';
    if(which==='similar')header=`<div class="side-section-title">${esc(t('viewer.referencePhoto'))}</div><div class="viewer-reference"><img src="/api/photos/${photoId}/thumbnail?v=2.2.10"><span>${esc($('#caption').textContent||'#'+photoId)}</span></div><div class="side-section-title">${esc(t('viewer.similarResults'))}</div>`;
...
window.openChildViewer=async(id,which,ids)=>{const q=`tab=${encodeURIComponent(which)}&ctx=${encodeURIComponent(which)}&ids=${encodeURIComponent((ids||[]).join(','))}`;try{await api(`/api/window/viewer/${id}?${q}`,{method:'POST'})}catch{open(`/viewer?photo=${id}&${q}&v=2.2.10`,'_blank','popup,width=1500,height=950')}};
...
(async()=>{applyTheme();await loadLocale();await loadPhoto(photoId,tab);updateViewerNav()})();
